# Migracao 12.06 - Configurando RabbitMQ com Docker
...
### Novidade desta aula
- Esta aula introduz a configuracao do RabbitMQ via `docker-compose.yml` no projeto `algasensors`.
- O arquivo `docker-compose.yml` foi mantido como no codigo do professor (sem alteracoes para a migracao de Spring Boot 4), pois nao depende das mudancas de API do Spring.
